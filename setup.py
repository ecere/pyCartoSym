...
pymodule = '_pycartosym' + sysconfig.get_config_var('EXT_SUFFIX')
   ...
   try:
      ...
      if not os.path.exists(artifacts_dir):
         ...
         prepare_package_dir([
            ...
            (os.path.join(cartosym_dir, 'obj', platform_str, 'bin', 'cs-canif' + exe_ext), os.path.join('bin', 'cs-canif' + exe_ext)),
            (os.path.join(rwd, 'cs_canif_wrapper.py'), os.path.join('bin', 'cs_canif_wrapper.py')),
...
if 'sdist' in commands:
   ...
   packages=['cartosym', 'cartosym.bin'] #, 'cartosym.examples']
   package_dir={
      'cartosym': artifacts_dir,
      'cartosym.bin': os.path.join(artifacts_dir, 'bin'),
      'cartosym.examples': os.path.join(artifacts_dir, 'examples'),
   }
   package_data={
   ...
   cffi_modules=[os.path.join('libCartoSym', 'bindings', 'py', 'build_cartosym.py') + ':ffi_cartosym']
